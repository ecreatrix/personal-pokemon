...
class PokedexUpdate extends BaseController {
    public function update() {
        $api      = ( new PokedexService() );
        $pokemons = [];

        for ( $pokedex_no = 100; $pokedex_no <= 100; $pokedex_no++ ) {
            ...
            if ( $species ) {
                foreach ( $species->varieties as $variety_info ) {
                    ...
                    $pokemons[] = $api->create_pokemon_custom( $species, $variety_info, $pokedex_no, $url_id );
                }
            }
        }
