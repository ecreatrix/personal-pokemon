...
###mkdir -p "$WGET/"pokedb-trans/_tmp
###rm "$WGET/"pokedb-trans/_tmp/*

find "$WGET/"pokedb/* \! -name ".DS_Store" -type f -maxdepth 0 | while read f; do
	NAME=$(basename "$f")
	NAME="${NAME%.*}"
	FPATH="$WGET/pokedb-trans"
	TEMP="$FPATH/_tmp/$NAME"
	...
	##convert "$TEMP-difference.png" -fuzz 5% -bordercolor white -border 1 -fill none -draw "alpha 0,0 floodfill" -shave 1x1 "$TEMP-removed-black.png"
	# create the matte
